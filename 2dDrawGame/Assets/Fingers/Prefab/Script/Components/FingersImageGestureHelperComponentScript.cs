...
namespace DigitalRubyShared
{
    ...
    public class FingersImageGestureHelperComponentScript : ImageGestureRecognizerComponentScript
    {
        ...
        Vector2 rotate(Vector3 pivotPoint, Vector3 pointToRotate, float angle)
        {
            ...
            float Ny = (pointToRotate.z - pivotPoint.z);
            angle = -angle * PIDiv180;
            result = new Vector2(Mathf.Cos(angle) * Nx - Mathf.Sin(angle) * Ny + pivotPoint.x, Mathf.Sin(angle) * Nx + Mathf.Cos(angle) * Ny + pivotPoint.z);
            return result;
        }
        private void UpdateLines()
        {
            ...
            if (idx >= 0 && idx < LineRenderers.Length)
            {
                ...
                LineRenderers[idx].alignment = LineAlignment.Local;
            }
